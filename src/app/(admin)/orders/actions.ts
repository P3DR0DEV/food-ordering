...
interface OrderStatusChangeResponse {
  id: string
  status: 'new' | 'cooking' | 'delivering' | 'delivered' | 'cancelled'
  total: number
  userId: string
...
export function useStatusChange(id: string, status: 'cooking' | 'delivering' | 'delivered' | 'cancelled') {
  const [data, setData] = useState<OrderStatusChangeResponse | null>(null)
  const [error, setError] = useState<Error | null>(null)
